...
export default function RootLayout() {
  ...
  const [hasLocPermission, setHasLocPermission] = useState<boolean | null>(
    null
  );
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener(
      "change",
      async (nextAppState) => {
        // 앱이 백그라운드나 inactive 상태였다가 active 상태로 전환될 때
        if (
          appState.current.match(/inactive|background/) &&
          nextAppState === "active"
        ) {
          console.log("포그라운드로 올라옴");
          // 포그라운드 권한 요청
  ...
  return (
    <ThemeProvider value={colorScheme === "dark" ? DarkTheme : DefaultTheme}>
      <SafeAreaView style={styles.safeArea}>
        <WebView
          ref={webviewRef}
          source={{ uri: "http://192.168.219.118:3000/search" }}
